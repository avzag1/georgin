...
export default function ShowcaseSlider () {
  ...
  const getBtnClass = (id: number) => 
    `w-45 h-9 border border-[#2D531A] lg:border-[#9AB973] text-[#2D531A] text-left lg:text-center text-xs lg:text-base rounded-4xl lg:rounded-none justify-between lg:justify-center items-center px-3 lg:px-0 ${
      toggle === id ? "lg:bg-[#9AB973] flex" : visible === true ? "lg:bg-white/0" : "hidden lg:block"
    }`;
    ...
    <div>
      <div className="flex mx-3">
        ...
        <div className="w-3/5 flex flex-col lg:flex-row justify-around mt-7 lg:mt-0 mb-2 mx-0 lg:mx-30 relative">
          ...
          <button onClick={toggleVisible} className="absolute right-4 top-2 block lg:hidden">
            <Image
          ...
          <button
            onClick={() => {setToggle(2)
              setVisible(false)}} className={getBtnClass(2)}
          >
          ...
          <button
            onClick={() => {setToggle(3)
                setVisible(false)}} className={getBtnClass(3)}
          >
          ...
          <button
            onClick={() => {setToggle(4)
                setVisible(false)}} className={getBtnClass(4)}
          >
