...
export default function ShoppingItem ({orderItem}: {orderItem: Product}) {
  ...
  const currentProductInStore = storeArray.find(p => p.title === orderItem.bouquet.title);
  const actualQuantityInStore = currentProductInStore?.quantity || 0;
  const actualPrice = orderItem.bouquet.actionPrice || orderItem.bouquet.price || 0;
  ...
  return (
    <div className="w-full h-[100] flex border-b mb-3">
      <div className="mr-0 lg:mr-5 pl-0 lg:pl-2 min-w-[90]">
        <Image src={orderItem.bouquet.image} alt="Букет" width={82} height={84} />
      </div>
      
      <div className="flex flex-col lg:flex-row">
        <div className="flex flex-col pb-3 pl-3 lg:pl-0 w-[145] lg:w-60">
          <div className="font-medium text-sm lg:text-base h-auto">{orderItem.bouquet.title}</div>
          <div className="text-xs lg:text-base h-auto">{orderItem.bouquet.description}</div>
        </div>
        ...
        <div className="flex justify-center items-center w-[56] lg:w-30 ml-8 lg:ml-0 scale-90 lg:scale-100">
          ...
          <div className="bg-[#eeeeee] py-1 min-w-[30] text-center w-[21]">{(order?.goods.find(product => product.bouquet.title === orderItem.bouquet.title)?.quantity)}</div>
      ...
      <div className="flex flex-col items-center ml-3 font-bold w-[61]">
        ...
        <div className="mt-5.5"><p>{(orderItem.quantity * actualPrice).toLocaleString('ru-RU')}</p></div>
      </div>
